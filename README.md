...
Technically, dbMAP essentially  builds an adaptive diffusion graph to denoise and encode data structure, and then visualizes it with UMAP or PaCMAP layouts. The diffusion
procedure learns a self-tuning diffusion metric, which can be encoded in a graph that approximates the Laplace-Beltrami Operator. This diffusion-basis addresses
UMAP and PaCMAP assumptions on the input connectivity graph, allowing high-resolution final embeddings. For more information on dbMAP,
 check our [preprint](https://www.researchgate.net/publication/341267564_Comprehensive_Visualization_of_High-Dimensional_Single-Cell_Data_With_Diffusion-Based_Manifold_Approximation_and_Projection_dbMAP).
...
## Installation and dependencies
   ...
   *NOTICE*: as of Feb 27 to March 4, dbMAP is undergoing active development to overhaul UMAP algorithm and discard the dependency, 
   and to optimize UMAP and PaCMAP layout procedures. To use the last version, please download dbMAP from TestPyPI:
